...
class Handler(ContentHandler,DTDHandler):
	...
	def __init__(self):
		...
		self.authorFile.write('authorId,name\n')
		self.authoredFile.write('authorId,pubId\n')
		self.articleFile.write('pubId,journal,number,volumn,month\n')
		self.inproceedingsFile.write('pubId,booktitle,editor\n')
		self.proceedingsFile.write('pubId,booktitle,editor\n')
		self.bookFile.write('pubId,isbn,publisher\n')
		self.incollectionFile.write('pubId,isbn,publisher\n')
		self.wwwFile.write('pubId,url,editor\n')
		self.publicationFile.write('pubId,pubKey,title,year,category\n')


	def startElement(self,name,attrs):
		self.currentNode = name
		if name in self.publication:
			self.tempData.update({'pubId':'\"'+str(self.pubId)+'\"'})
	...
	def endElement(self,name):	
		self.currentNode = ''
		if name in self.publication:
			self.entry =  self.tempData.get('pubId','')+','+self.tempData.get('pubKey','')+','+self.tempData.get('title','')+','+self.tempData.get('year','')+',\"'+name+'\"\n'
			self.publicationFile.write(self.entry.encode('utf-8'))
			if name == 'article':
				self.entry = self.tempData.get('pubId','')+','+self.tempData.get('journal','')+','+self.tempData.get('number','')+','+self.tempData.get('volumn','')+','+self.tempData.get('month','')+'\n'
				self.articleFile.write(self.entry.encode('utf-8'))
			elif name == 'inproceedings':
				self.entry = self.tempData.get('pubId','')+','+self.tempData.get('booktitle','')+','+self.tempData.get('editor','')+'\n'
				...
				self.entry = self.tempData.get('pubId','')+','+self.tempData.get('isbn','')+','+self.tempData.get('publisher','')+'\n'
				self.bookFile.write(self.entry.encode('utf-8'))
			elif name == 'incollection':
				self.entry = self.tempData.get('pubId','')+','+self.tempData.get('isbn','')+','+self.tempData.get('booktitle','')+','+self.tempData.get('publisher','')+'\n'
				self.incollectionFile.write(self.entry.encode('utf-8'))
			elif name == 'www':
				self.entry = self.tempData.get('pubId','')+','+self.tempData.get('url','')+','+self.tempData.get('editor','')+'\n'
				self.wwwFile.write(self.entry.encode('utf-8'))
